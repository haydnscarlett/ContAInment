...
void Player::savePlayerData()
{
  ...
  for (int i : range)
  {
    if (inventory[i].getMyGameObject().getMySpriteId() >= 0 &&
        inventory[i].getMyGameObject().getMySpriteId() < 10)
    {
      tmp = "0";
      data.append(tmp);
    }
  ...
  if (player_data.open("/userdata/player.dat", ASGE::FILEIO::File::IOMode::WRITE))
  {
...
void Player::movePlayer(double animation_counter)
{
  if (moving)
  {
    if (animation_counter > 0.08)
    {
      ...
      {
        case NORTH:
          if (sprite_index + 1 > 15 || sprite_index + 1 < 12)
          {
            sprite_index = 12;
          }
          else
          ...
          break;
        case EAST:
          if (sprite_index + 1 > 11 || sprite_index + 1 < 8)
          {
            sprite_index = 8;
          }
          else
          ...
          break;
        case SOUTH:
          if (sprite_index + 1 > 3 || sprite_index + 1 < 0)
          {
            sprite_index = 0;
          }
          else
          ...
          break;
        case WEST:
          if (sprite_index + 1 > 7 || sprite_index + 1 < 4)
          {
            sprite_index = 4;
          }
          else
