...
void Room::moveRoom(Player* player)
{
  if (player->isMoving())
  {
    ...
    for (int i = 0; i < number_of_clues; i++)
    {
      ...
      my_clues[i].setMyGameObject(
        GameObject(my_clues[i].getMyGameObject().getMySpriteId(), new_location));
    }
    for (int i = 0; i < number_of_items; i++)
    {
      ...
      my_items[i].setMyGameObject(
        GameObject(my_items[i].getMyGameObject().getMySpriteId(), new_location));
    }
    for (int i = 0; i < my_puzzle.getNumberSwitches(); i++)
    {
      ...
      my_puzzle.getMySwitches()[i].setMyOffGameobject(GameObject(
        my_puzzle.getMySwitches()[i].getMyOffGameobject().getMySpriteId(), new_location));
      my_puzzle.getMySwitches()[i].setMyOnGameobject(
        GameObject(my_puzzle.getMySwitches()[i].getMyOnGameobject().getMySpriteId(), new_location));
    }
    for (int i = 0; i < my_puzzle.getNumberMovables(); i++)
    {
      if (!my_puzzle.getMyMovables()[i].collisionCheck(
            player->getPlayerGameobject().getMyLocation(), player->getDirection(), false))
      {
    ...
    for (int i = 0; i < number_of_exits; i++)
    {
      ...
      my_exits[i].setMyExitGameobject(
        GameObject(my_exits[i].getMyExitGameobject().getMySpriteId(), new_location));
    }
  }
}
...
bool Room::checkCollisions(Player* player, bool add_item_check)
{
  ...
  for (int i = 0; i < (my_gridsize_x * my_gridsize_y); i++)
  {
    if (my_foreground[i].collisionCheck(
          player->getPlayerGameobject().getMyLocation(), player->getDirection(), add_item_check))
    {
      colliding = true;
    }
  }
 ...
 *            room
 */
int Room::checkForInteractables(Player* player, std::string* text_to_display, bool* power_on)
{
  int colliding = false;
  ...
  for (int i = 0; i < number_of_clues; i++)
  {
    if (my_clues[i].getMyGameObject().collisionCheck(
          player->getPlayerGameobject().getMyLocation(), player->getDirection(), false))
    {
      if (my_clues[i].addClueToPlayer(player->getCluesFound(), player->getNumberCluesFound()))
      {
      ...
      switch (my_clues[i].getClueID())
      {
        case 0:
          *text_to_display = "There is no power.";
          break;
        case 1:
          *text_to_display = "There is a spare reactor fuse in the lab coat in the garden, did you "
                             "leave \n"
                             "it there on purpose?";
          break;
        case 2:
        ...
        case 5:
          *text_to_display = "Reading list: Reprogrammimng the brain, Modern Neuroscience  \n"
                             "Advanced AI, AI Pattern Recogniton, Printing Synthetic organs, The "
                             "synthetic Human.";
          break;
        case 6:
        ...
        case 7:
          *text_to_display = "What is this? We were using human brains, \nno no no, this cannot be "
          ...
          break;
        case 8:
          *text_to_display = "Power continues to cut out, i'm starting to think it's not a "
                             "coincidence, \n"
                             "i'll keep the key to the reactor in my safe until the issue is "
                             "resolved.";
          break;
        case 9:
  ...
  if (my_puzzle.checkPuzzleCompleted(player, power_on) && !*power_on && room_id == REACTOR)
  {
    *power_on = true;
...
void Room::resetRoomPosition(Point2D distance)
{
  ...
  for (int i = 0; i < my_puzzle.getNumberSwitches(); i++)
  {
    ...
    new_location.x += distance.x;
    new_location.y += distance.y;
    my_puzzle.getMySwitches()[i].setMyOffGameobject(
      GameObject(my_puzzle.getMySwitches()[i].getMyOffGameobject().getMySpriteId(), new_location));
    my_puzzle.getMySwitches()[i].setMyOnGameobject(
      GameObject(my_puzzle.getMySwitches()[i].getMyOnGameobject().getMySpriteId(), new_location));
  }
  for (int i = 0; i < my_puzzle.getNumberMovables(); i++)
...
bool Room::checkMovableCollisions(Player* player, bool add_item_check)
{
  for (int i = 0; i < my_puzzle.getNumberMovables(); i++)
  {
    ...
    if (my_puzzle.getMyMovables()[i].collisionCheck(
          player->getPlayerGameobject().getMyLocation(), player->getDirection(), true))
    {
...
void Room::setupFloorStandard()
{
  ...
  auto left_side =
    static_cast<float>(GAME_WIDTH * 0.5 - (GRID_SIZE * (static_cast<float>(my_gridsize_x) * 0.5f)));
...
void Room::setupWalls()
{
  ...
  auto wall_left_side = static_cast<float>(
    (GAME_WIDTH * 0.5) - (GRID_SIZE * (static_cast<float>(my_gridsize_x) * 0.5f)) - GRID_SIZE);
  auto wall_top_side = static_cast<float>(
    (GAME_HEIGHT * 0.5) - (GRID_SIZE * (static_cast<float>(my_gridsize_y) * 0.5f)) - GRID_SIZE);
  ...
  for (int i = 0; i < (((my_gridsize_x + my_gridsize_y) * 2) + 4); i++)
  {
    ...
    else if (i > my_gridsize_x + 1 && i < my_gridsize_x + my_gridsize_y + 2)
    {
      wall_left_side = static_cast<float>(
        (GAME_WIDTH)*0.5 - (GRID_SIZE * (static_cast<float>(my_gridsize_x) * 0.5f)) - GRID_SIZE);
    ...
    else if (i > my_gridsize_x + my_gridsize_y + 2 && i < (my_gridsize_x + (my_gridsize_y * 2)) + 2)
    {
      wall_top_side += GRID_SIZE;
      Point2D new_location;
      new_location.x = wall_left_side;
